...
import com.johngrib.neovim.annotation.NotificationHandler
import com.johngrib.neovim.annotation.RequestHandler
import com.johngrib.neovim.rpc.*
...
class Client(connection: NeovimConnection, onClose: (Throwable?) -> Unit) {
    ...
    init {
        receiver.start({ msg ->
            ReceiverChannel.trySend(this to msg).isSuccess
        }, onClose)
    }
    ...
    suspend fun request(method: String, args: List<Any?>): Response {
        ...
        val handler: (Response) -> Unit = { rsp: Response ->
            ret.set(rsp)
            channel.trySend(rsp).isSuccess
        }
        resHandlers[req.id] = handler
        SenderChannel.trySend(this to req).isSuccess
    ...
    fun registerHandler(obj: Any) {
        for (function in obj::class.memberFunctions) {
            ...
            if (reqAnnotation != null) {
                ...
                reqHandlers[reqAnnotation.name] = { request ->
                    function.call(obj, convertParam(function.parameters[1].type, request))
                }
            }
            ...
            if (notiAnnotation != null) {
                ...
                notiHandlers[notiAnnotation.name] = { notification ->
                    function.call(obj, convertParam(function.parameters[1].type, notification))
                }
            }
        }
    }
    ...
    private fun handleMessage(msg: Message) {
        if (msg is Response) {
            ...
            SenderChannel.trySend(this to rsp).isSuccess
